  ...
  await FFprobeKit.executeAsync(
    ...
    (session) async {
      ...
      if(ReturnCode.isSuccess(returnCode)) {
        ...
        completer.complete(result); /// 데이터만 보관하고,
        // File(audioFilePath).deleteSync(recursive: true); /// 파일은 삭제.
      } else {
...
Future<bool> compareAudios(File filteredAudioCt, File filteredAudioCp) async {
  ...
  int matchCount = 0;
  double allowableSeconds = 0.3;

  getDataFromAudioFile('ct', filteredAudioCt).then((audioDataCt) {
    printGreen("//////////// CT COUNTS = '${audioDataCt.length}' ////////////");
    printGreen(audioDataCt.toString());

    getDataFromAudioFile('cp', filteredAudioCp).then((audioDataCp) {
...
Future<String> mergeAudios(String inputName1, String inputName2, String inputName3) async {
  ...
  fileToByteData(inputName1).then((input1) {
