...
class VideoWaveFormsState extends ConsumerState<VideoWaveForms> with TickerProviderStateMixin {
  ...
  void _initializeControllers() {
    ...
    waveformPlayerController.onPlayerStateChanged.listen((event) {
      ...
      if(event.isInitialised) {
        ...
        waveformPlayerController.startPlayer(finishMode: FinishMode.pause);
        audioPlayer.resume();
      }
        ...
        if(!isRecording) {
          ...
          waveformPlayerController.startPlayer(finishMode: FinishMode.pause);
          audioPlayer.stop().then((_) => audioPlayer.resume());
        }
      }
    });
  }
  ...
  void _initializeControllers4Record() {
    ...
    waveformPlayerController4Record.onPlayerStateChanged.listen((event) {
      if(event.isInitialised) {
      ...
      if(recorderController.recorderState.isRecording)  {
        AudioWaveformsInterface.instance.getDecibel().then((decibel) {
          if(mounted
              ...
              && duration < Duration(milliseconds: widget.duration)) {
            if(Platform.isAndroid && decibel! > 30000) {
              setState(() => listClapIcons.add(clapIcon(getWidgetPosition(clapIconKey))));
            }
            else if(Platform.isIOS && decibel! > 2 && !claps4Ios.contains(decibel)) {
              setState(() {
  ...
  void recording() {
    ...
    waveformPlayerController
      ..pausePlayer()
      ..seekTo(0);
    waveformPlayerController4Record.startPlayer(finishMode: FinishMode.pause);
    audioPlayer4Record.resume();
  ...
  ///
  ///
  void compareAfterFilterApplied() {
    if(mounted) {
      setState(() {
        isComparing = true;
      });
    ...
    applyFilter('comparison', File(recordFilePath)).then((filteredRecordFile) {

      // Control Recorded File (녹음된 파일) == 'File(widget.originAudioFile.path)'
      // Comparison Recorded File (필터 처리된 녹음된 파일) == 'filteredRecordFile'
      compareAudios(File(widget.originAudioFile.path), filteredRecordFile).then((bool result) {
        ...
        if(mounted) {
          ...
          if(result) {
            ...
            //TEMP// File(recordFilePath).deleteSync(recursive: true);
            //TEMP// File(filteredRecordFile.path).deleteSync(recursive: true);
